    ...
    for assessment_name in the_payload.assessments:
        ...
        if the_assessment.access_log_enabled:
            try:
                ...
                client_info = AccessLog(payload_id, referrer, user_agent, ip_address)
...
def get_generic_callback():
    ...
    if request.method == 'POST':
        try:
            app.logger.info("request.form.get('payload_id', 0): {}".format(request.form.get('payload_id', 0)))

            javascript_name = urllib.unquote(unicode(request.form.get('javascript_name', '')))

            # If they don't set a URL, ignore it
            ...
            payload = Payload.query.filter_by(id=int(request.form.get('payload_id', 0))).first()

            # If it's a rogue capture, log it anyway.
            if payload is None:
                client_info = GenericCollector(0, javascript_name, url, referrer, data)
            else:
                # Create the capture with associated assessment/payload
                client_info = GenericCollector(payload.id, javascript_name, url, referrer, data)

            db.session.add(client_info)
            db.session.commit()
            # Email users subscribed to the Payload's Assessment
            email_subscription(payload.id, url, client_info, 'generic_collector')
        except Exception as e:
            app.logger.warn("Exception in /generic_callback {}\n\n{}".format(Exception, str(e)))
            import traceback
            traceback.print_exc()
